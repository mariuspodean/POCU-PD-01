...
#Convert isocodes to country name
count = []
for c in range(len(raw_data)):
  count.append(countries.get(alpha_2 = raw_data[c][0]).name)
print(count)

def preparaded_dataset(old_data):
 # ''' Preparing the dataset '''
  dataset = {}
  for data in range(len(old_data)):
    years_pet = []
    for desc in range(len(description[1])):
        if old_data[data][1][desc] != ': ':
            years_pet.append({"year" : description[1][desc], "coverage": raw_data[data][1][desc]})
    dataset[count[data]] = years_pet
  return dataset
    
dataset = {}
dataset = preparaded_dataset(raw_data)
