...
def get_country_name(country_code):
    country_information = pycountry.countries.get(alpha_2 = country_code)
    if country_information != None:
        return(country_information.name)
    else:
         return(country_code)
...
def process_information(description,raw_data):
    processed_data = {}
    years_list = []
    ...
    for entry in raw_data:
        ...
        for coverage in entry[1]:
            if coverage is not no_data:
                processed_data[country].append(dict(year = years_list[year], coverage = get_numeric_value(coverage)))
            year += 1      
...
def get_country_data(dataset,country):
    ...
    for entry in dataset[country]: 
        country_tuple = (entry['year'], entry['coverage'])
        result.append(country_tuple)
...
def get_year_data(dataset,year):
    ...
    for entry in dataset:
        for item in dataset[entry]:
            if (item['year'] == year ):
                year_tuple = (entry,item['coverage'])
                result.append(year_tuple)
    year_data[year] = result
    return year_data

def perform_average(iterable):
    average = 0
...
#perform_average(get_country_data(process_information(description,raw_data),'UK'))
#get_year_data(process_information(description,raw_data),'2011')   
#get_country_data(process_information(description,raw_data),'Latvia')
average = get_year_data(process_information(description,raw_data),'2011')
print(perform_average(average))
